...
LiquidCrystalRus::LiquidCrystalRus(uint8_t rs, uint8_t enable, uint8_t d0,
...
void LiquidCrystalRus::init(uint8_t fourbitmode, uint8_t rs, uint8_t rw,
...
void LiquidCrystalRus::setDRAMModel(uint8_t model) { _dram_model = model; }
...
inline void LiquidCrystalRus::command(uint8_t value) { send(value, LOW); }
  ...
  if (value >= 0x80) {  // UTF-8 handling
    if (value >= 0xc0) {
      ...
      if (!utf_hi_char && (value == 1))
        ...
        send(pgm_read_byte_near(utf_recode + value + (utf_hi_char << 6) - 0x10),
             HIGH);
    }
  } else
    send(out_char, HIGH);
